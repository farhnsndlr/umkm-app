...
const serviceDetailsPage: React.FC = () => {
  ...
  const handleContact = () => {
    if (!service || !service.phoneNumber) return;
    const waUrl = `https_://wa.me/${service.phoneNumber}`;
        ...
        <div className="flex flex-col h-full overflow-y-auto">
          ...
          <div className="p-6 md:p-10">
            ...
            <div className="flex items-center gap-2 my-3 text-gray-700">
              <span className="font-bold text-lg">{service.ratings}</span>
            ...
            {activeTab === "menu" && (
              <div className="space-y-10">
                {service.menu && service.menu.images.length > 0 && (
                  <div>
                    <h3 className="text-2xl font-bold mb-4">Menu</h3>
                    <div className="space-y-4">
                      {service.menu.images.map((imgSrc, index) => (
                        <img
                          key={`menu-${index}`}
                          src={imgSrc}
                          alt={`Menu ${service.title} ${index + 1}`}
                ...
                {service.tampilan && service.tampilan.images.length > 0 && (
                  <div>
                    <h3 className="text-2xl font-bold mb-4">Tampilan</h3>
                    <div className="grid grid-cols-2 gap-4">
                      {service.tampilan.images.map((imgSrc, index) => (
                        <img
                          key={`tampilan-${index}`}
                          src={imgSrc}
                          alt={`Tampilan ${service.title} ${index + 1}`}
                ...
                {(!service.menu || service.menu.images.length === 0) &&
                  (!service.tampilan ||
                    service.tampilan.images.length === 0) && (
